...
class NoneUser:
    ...
    @classmethod
    def get(cls):
        if not cls.the_none_user:
            cls.the_none_user = User.objects.get(id=settings.NONE_ID)
        return cls.the_none_user
...
class PettycashTransaction(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.src:
            self.src = NoneUser.get()
        if not self.dst:
            self.dst = NoneUser.get()
